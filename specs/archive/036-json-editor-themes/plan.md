...
**D5 — Rendering: a two-layer file-driven mapping over the retained default base.** (Revised 2026-08-23 after review; research E10 records the defects in the original single-layer version.) The renderer applies the selected definition as inline `--mm-theme-*` custom properties (six tokens + typeface) on the app container, mapped onto Crepe's variables by TWO rules in `themes.css`:

- **Derived tones — first, lowest precedence.** surface-low, on-surface, and on-surface-variant map from the file's surface/foreground/outline in a rule placed BEFORE the preset blocks at equal-or-lower specificity. The presets' hand-tuned values therefore win for the five default names, keeping unedited defaults pixel-identical to the previous version (the original single layer overrode them with coarser mappings: rustic surface-low `#fcefce`→`#fdf3d9`, on-surface `#201b13`→`#1f1b16`, on-surface-variant `#4f4539`→`#817567`; scholarly/monotone similar). Themes without a preset block — user-created files and the emergency appearance — take these mappings from this rule.
- **Curated tokens + typeface — last, highest precedence.** background, on-background, surface, outline, primary, inline-code/error, and both font variables map from the six curated tokens in a rule placed AFTER the preset blocks and qualified with `[data-theme]`, so it ties with the `[data-theme]`-qualified monotone blocks at (0,4,0) and wins by source order. File tokens drive their direct counterparts for EVERY discovered theme, monotone included (a plain `.app-container[data-editor-theme]` selector loses to those blocks, which broke editing monotone files).

For unedited defaults the file values equal the preset blocks' own declarations for exactly these direct counterparts, so the curated layer winning changes nothing visually. Editing a default file changes every property the six-token mapping drives while derived tones stay base-driven (the schema cannot express them — Complexity Tracking #1); a cascade unit test (`tests/renderer/themesCssCascade.test.ts`) pins the resolved values per theme and appearance. When nothing resolves, the container renders `data-editor-theme="default"` with both layers' `var()` fallbacks — the full rustic palette including its fine derived tones + Inter: the single FR-001 exception, never listed or selectable.
...
**D9 — Migration mirrors spec 023 detection.** At startup, after seeding: read the raw `.settings` section; if it stores a valid six-token `editorColors` record, compare (case-insensitive hex, like `colorsMatch`) plus the stored two-valued `editorFont` against the five defaults' colours-and-choice table (research E1): an exact match repairs `editorTheme` to that file stem (no duplicate artifact); no match creates `migrated-custom.json` holding those colours in BOTH sets plus the typeface stack for the stored choice — never overwriting an existing file. An invalid/missing legacy `editorFont` behaves as `'sans-serif'`, mirroring the old runtime fallback, so what users saw is what migrates; invalid/absent legacy colours make the step a complete no-op (the old app ignored them and rendered the stored preset). Every acting pass also drops the legacy `editorColors`/`editorFont` keys in the same atomic write — otherwise a stale palette would re-yank the user's later manual selection on every restart (FR-006), and FR-009 requires the fields to become inert afterwards. Both outcomes are idempotent: once the fields are gone, later runs are no-ops. Note `scholarly`'s legacy choice is `sans-serif` while its file typeface is the Arial stack it renders today — matching uses the choice, rendering uses the stack (research E1/E2).

## Complexity Tracking

> Two deliberate deviations from the artifacts' letter, both recorded where AGENTS.md requires.

1. **Retained name-keyed base CSS for the five default names (bends FR-001's "no theme defined anywhere else").** The six-token schema cannot express eleven derived Crepe tones that today's preset blocks hard-code (hover/selected/inverse/on-inverse/surface-low/on-surface/on-surface-variant/secondary/on-secondary/inline-area, plus Scholarly's accent-coloured headings, research E3). Dropping those blocks would visibly change selection, hover, muted-text, and heading colours of every default theme, violating FR-002/US2-S4 ("rendered appearance … compared against the previous version … looks the same as before"), which is an explicit acceptance scenario with a comparison test. The chosen layering (plan D5, revised after review) keeps the blocks as the base for exactly those derived tones: the file-driven layers map the six curated tokens for every discovered theme and yield the derived tones to these blocks for the five default names. Identity, tokens, typeface, and every user-facing customisation come solely from the files; the base CSS fires only for the five default names, offers no second customisation path, and goes dormant if a default file is renamed away. Simpler alternative rejected: deleting the blocks (single flat layer) in favour of not regressing the defaults' appearance; also rejected after review: a single always-winning overlay (pixel-identity false on three derived tones per default) and a schema extension adding derived tones to theme files (validation and migration complexity for values users never see in the dialog). Recorded in the spec's Clarifications (2026-08-23).
