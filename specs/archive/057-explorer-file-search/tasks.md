...
- [X] 3.1 Write `tests/e2e/explorer-search.spec.ts`: live filtering on typing (US1); matches inside collapsed-but-loaded folders become visible with ancestors shown (FR-004; never-loaded folders are out of scope per FR-007, recorded in spec.md); non-matching entries hidden (FR-005); folder-name match shows the folder only (FR-006); no-match empty state (FR-009); activating a match opens the file exactly as unfiltered activation does, including duplicate-tab focus (US3/FR-010); activating a matching folder focuses it like unfiltered activation (FR-011, folded into the restore scenario); clearing restores expansion and selection exactly (US2/FR-008); Escape clears and refocuses the tree (FR-014); create/rename/delete flows still work while filtered (US3-3); the term resets on workspace change and restart (FR-013); a 5,000-entry workspace filters without perceptible lag (FR-012/SC-002)
