...
**Evidence**: The serializer throws when the live document contains a node the active serializer cannot handle (`node_modules/@milkdown/transformer/lib/index.js:213-216`), so `getMarkdown()` on the toggle path (`instancePool.ts:63-69`, called from `dirty.ts` helpers) can throw on such documents; which constructs a document can contain depends on the configured processor assembled from the syntax options (`markdownSyntaxRuntime.ts:73-80`). The precise trigger conditions are not reproduced yet and belong to the implementation phase's reproduction work. Selection creation can throw as described in R3 (ProseMirror's `TextSelection.create` raises RangeError when the resolved position cannot host a text selection; https://prosemirror.net/docs/ref/#model.TextSelection^create). With no error boundary in the renderer, such an exception kills the interaction or the tree, presenting as "frozen". A boundary keeps the previous usable surface and satisfies FR-006 and Principle IV's quiet-error rule.
