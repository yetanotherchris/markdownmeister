...
- [x] 4.2 `tests/renderer/useSettingsState.test.tsx`: verified against the new wiring; no edit needed (its word wrap coverage targets the hook and handler, which are unchanged)
...
- [x] 4.5 `tests/e2e/editor-theme.spec.ts`: left-edge alignment, accessible name without visible label; `json-editor-themes.spec.ts` and `settings.spec.ts` verified as needing no edit (they look the combobox up by test id and accessible name, both unchanged)
