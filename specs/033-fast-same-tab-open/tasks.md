...
- [ ] **T013** New `tests/e2e/open-performance.spec.ts` driving the built app:
  - SC-001: open a generated document into a clean active tab repeatedly, read `openDurations` from `window.__mmOpenPerformance`, assert p95 within the 250 ms target multiplied by a documented CI tolerance (×4 when `process.env.CI` is set, ×1 locally).
