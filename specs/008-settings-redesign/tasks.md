...
- [ ] T009 [US1] Redesign `src/renderer/chrome/SettingsDialog.tsx` with General/Theme navigation, area-specific panels, accessible native switches, and expanded focus trapping.
- [ ] T010 [US1] Implement the wider responsive sidebar, selected navigation state, panel layout, and switch styles in `src/renderer/chrome/settings.css`.
- [ ] T011 [US1] Wire new settings values and dialog callbacks in `src/renderer/App.tsx` and `src/renderer/hooks/useSettingsState.ts`.
- [ ] T012 [US1] Extend immediate-persistence handler coverage in `tests/renderer/useSettingsState.test.tsx`.
