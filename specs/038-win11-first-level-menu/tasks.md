...
- [x] T007 [US2] Create tests/main/storeHandoff.test.ts proving parity: alias-shaped argv ([…WindowsApps\markdownmeister.exe, folder]) vs classic verb-shaped argv produce identical extractTargetFromArgv/classifyOsTarget outcomes for a real temp folder; adversarial cases (deleted folder, reserved device names, nonexistent UNC host, non-string) fail closed with path-free messages. Commit `test(038)`.
