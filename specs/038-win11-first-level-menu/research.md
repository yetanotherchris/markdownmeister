...
**Namespace note**: Terminal uses `desktop5:` for `ItemType`/`Verb` inside a `desktop4:FileExplorerContextMenus`; Microsoft's own how-to uses all-`desktop4`. Both namespaces are long-shipped schemas. This project follows Microsoft's documented form (all `desktop4`) to match the canonical instruction; both are recorded here so the choice is traceable.
...
**Alternatives considered**: /EHsc C++ try/catch alone — does not catch access violations reliably in mixed code and lets SEH escape from OS callbacks; WRL without guards still lets implementation bugs crash the host. Rejected as insufficient alone (both are still used internally; SEH is the boundary).
