...
**Independent Test**: On a stock macOS machine with default Gatekeeper settings, download the published archive, launch the app by double-click, and confirm it opens with no warning and no extra steps.

**Acceptance Scenarios**:

1. **Given** the published macOS artifact, **When** the user opens it on a stock macOS machine, **Then** the app launches without the unidentified-developer or malware-verification warning.
2. **Given** the artifact is downloaded through a browser (quarantined), **When** the user opens it, **Then** Gatekeeper accepts it and the app launches normally.
3. **Given** the app is installed via Homebrew, **When** the user launches it, **Then** it opens with the same warning-free behaviour.
4. **Given** both supported architectures, **When** each published artifact is opened on its matching machine, **Then** both launch without a warning.
...
**Independent Test**: Force signing or notarization to fail in a test pipeline and confirm no macOS artifact is published for that version, with the failure clearly reported.

**Acceptance Scenarios**:

1. **Given** signing fails, **When** the release pipeline runs, **Then** no macOS artifact is published and the failure is reported to the maintainer.
2. **Given** notarization is rejected, **When** the pipeline runs, **Then** the release does not publish that macOS build and the rejection reason is surfaced.
3. **Given** a failure, **When** the release ends, **Then** the previously published macOS version remains the current one.
4. **Given** the direct-download artifacts fail, **When** the release ends, **Then** the Homebrew definition is not updated to point at a missing or invalid artifact.
...
- An Intel machine runs the arm64 artifact (or vice versa): the Store-independent artifacts remain architecture-correct; the wrong architecture must not be offered.
...
- **FR-002**: Every published macOS artifact MUST be notarized by Apple and have the notarization ticket stapled to the app bundle.
...
- **FR-005**: All nested executables and frameworks inside the bundle MUST be signed so the bundle verifies as a whole.
...
- **FR-007**: If signing, bundle verification, or notarization fails, the release MUST NOT publish a macOS artifact for that version and MUST surface the failure.
...
- **Release artifact**: The versioned macOS archive (per architecture) published on the releases page and consumed by Homebrew.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: In 100% of release verification runs, the published macOS artifact passes Gatekeeper assessment with no user intervention.
...
- **SC-003**: Zero unsigned or unnotarized macOS artifacts are published; a signing/notarization failure blocks the release.
...
- **No Mac ownership required**: Signing and notarization can run on CI macOS runners using credentials and an App Store Connect API key created in Apple's web portal. The maintainer does not currently own a Mac, so local verification is limited to what CI and spot checks can provide.
...
- **Verification split**: Automated checks cover signature presence, bundle integrity, and pipeline failure behaviour; the actual warning-free launch on real hardware is verified manually (the maintainer may need a borrowed or loaner Mac for the final spot check).
