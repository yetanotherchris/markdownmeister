 ...
 * and by spec 054 to a labelled checkbox: the two-state control defaulting off
 * (FR-012), today's behaviour when off, wrapped presentation when on, immediate
 * application to open source views (FR-011), mid-edit safety (FR-013),
 * persistence and malformed-value recovery, the far-right header-bar position
 * and visible state (FR-009, FR-010), absence from Settings (FR-008), an
 * untouched visual editor (FR-014), and a typing-latency smoke on a large
 * wrapped document (SC-005).
 */
...
test('FR-009 the toggle sits at the far right of the header bar, back button far left', async () => {
  ...
  const positions = await bar.evaluate((toolbar) => {
    ...
    const toggle = toolbar.querySelector<HTMLElement>('.source-word-wrap')
...
test('FR-010/FR-012 the checkbox communicates its state and defaults to off', async () => {
  await openSourceView('alpha.md')

  const checkbox = wordWrapToggle()
  await expect(checkbox).not.toBeChecked()
  await expect(window.locator('.source-word-wrap')).toHaveText('Word Wrap')
})

test('FR-005 the control is a native checkbox whose checked state tracks wrapping', async () => {
  await openSourceView('alpha.md')
  ...
  // Toggling back off returns the checked state to match.
  await toggleWordWrap()
...
test('FR-010 the checkbox is operable from the keyboard', async () => {
  await openSourceView('alpha.md')
  await expect.poll(sourceOverflows).toBe(true)
  ...
  await window.keyboard.press('Space')
  await expect.poll(sourceOverflows, { timeout: 1000 }).toBe(true)
  await expect(wordWrapToggle()).not.toBeChecked()
})
...
test('US1 the choice persists across a restart', async () => {
  ...
  await openSourceView('alpha.md')
  await expect.poll(sourceOverflows).toBe(false)
  // The restored state is visible on the checkbox itself.
  await expect(wordWrapToggle()).toBeChecked()
})
