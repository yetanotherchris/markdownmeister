...
- After the PR is created, **before merging**, launch 5 agent-based code reviews.
...
- After all 5 reviews have posted, **address their findings before merging**:
  ...
  - After addressing findings, re-run `npm run lint`, `npm run typecheck`,
    `npm run test`, and `npm run test:e2e`; the PR is not ready to merge until
    all four are green.
